...
from core.Constants import ss


class Vec3Make(BaseNode):
    def __init__(self, scene, x=0, y=0):
        super(Vec3Make, self).__init__(scene, x=x, y=y)

        self.change_title("make_vec3")
        ...
        self.z = self.add_input(socket_types.FloatSocketType(self, "z input"), "z input")
        self.z.set_initial_value(0)

        self.result_vector = self.add_output(socket_types.Vector3SocketType(self), "result")
        self.result_vector.set_initial_value([0.0, 0.0, 0.0])

        self.set_auto_compute(False)

    def compute(self):
        result = [self.x.get_value(), self.y.get_value(), self.z.get_value()]
        try:
        ...
        if self.z.is_connected():
            self.result_vector.socket_type.z_spin.setStyleSheet(ss.socket_ui_connected)
            self.result_vector.socket_type.z_spin.setEnabled(False)
        else:
            self.result_vector.socket_type.z_spin.setStyleSheet("")
            self.result_vector.socket_type.z_spin.setEnabled(True)
